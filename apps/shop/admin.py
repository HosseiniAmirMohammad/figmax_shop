...
class ProductForm(forms.ModelForm):
    price = forms.CharField(
        label="قیمت (تومان)",
        help_text="مثلاً: 312,000 یا 312000",
        required=True
    )
...
class ProductAdmin(admin.ModelAdmin):
    form = ProductForm
    list_display = ('title', 'price_display', 'stock', 'is_active', 'is_featured', 'category', 'created_at')
...
class BannerSlideAdmin(admin.ModelAdmin):
    ...
    def preview_image(self, obj):
        if obj and obj.image:
            return format_html('<img src="{}" width="120" height="70" style="object-fit:cover; border-radius:4px;"/>', obj.image.url)
        return "-"
...
class HomeSettingsAdmin(admin.ModelAdmin):
    fieldsets = (
        ...
        ('بخش هیرو - رنگ‌ها', {
            'fields': ('hero_title_color', 'hero_title_first_word_color', 'hero_title_second_word_color', 'hero_subtitle_color'),
            'description': 'رنگ‌های مورد استفاده در بخش هیرو',
            'classes': ('collapse',),
        }),
        ('بخش هیرو - فونت و اندازه', {
            'fields': ('hero_title_font_size', 'hero_title_font_weight', 'hero_subtitle_font_size', 'hero_banner_height', 'hero_banner_overlay_opacity'),
            'description': 'اندازه و ضخامت فونت‌ها و ارتفاع بنر',
            'classes': ('collapse',),
        }),
        ('بخش CTA (دعوت به اقدام)', {
            'fields': ('cta_title', 'cta_subtitle', 'cta_button_text', 'cta_button_link', 'cta_background_image', 'cta_height'),
            'description': 'تنظیمات بنر CTA'
        }),
        ('رنگ‌های بخش CTA', {
            'fields': ('cta_title_color', 'cta_subtitle_color', 'cta_button_color', 'cta_button_text_color', 'cta_overlay_opacity', 'cta_border_radius'),
...
class OrderAdmin(admin.ModelAdmin):
    list_display = ('order_number', 'user', 'status', 'tracking_code_display', 'total_price_display', 'created_at')
